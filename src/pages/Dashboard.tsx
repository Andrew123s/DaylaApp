      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 sm:mb-8 gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">
              ...
              {user?.preferences && <Sparkles className="inline h-5 w-5 sm:h-6 sm:w-6 text-yellow-500 ml-2" />}
            </h1>
          ...
          <button
            ...
            className="flex items-center justify-center space-x-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white px-4 py-2 sm:px-6 sm:py-3 rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-lg hover:shadow-xl w-full sm:w-auto"
          >
        ...
        {personalizedRecommendations.length > 0 && (
          <div className="mb-6 sm:mb-8">
            <h2 className="text-xl font-bold text-gray-900 mb-3 sm:mb-4 flex items-center space-x-2">
              <Sparkles className="h-5 w-5 text-yellow-500" />
              <span>Recommended for You</span>
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              {personalizedRecommendations.map((rec, index) => (
                <div key={index} className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-4 sm:p-6 border border-blue-200">
                  <h3 className="font-semibold text-gray-900 mb-2">{rec.title}</h3>
                  <p className="text-gray-600 text-sm mb-3 sm:mb-4">{rec.description}</p>
                  <div className="flex flex-wrap gap-2">
                    {rec.destinations.map((dest, i) => (
                      <span key={i} className="px-2 py-1 sm:px-3 sm:py-1 bg-white text-blue-800 text-xs rounded-full font-medium">
                        {dest}
                      </span>
                    ))}
                  </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-white/20 shadow-sm">
            <div className="flex items-center">
              ...
              <div className="ml-3 sm:ml-4">
                ...
                <p className="text-xs sm:text-sm text-gray-600">Collaborators</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-white/20 shadow-sm">
            <div className="flex items-center">
              ...
              <div className="ml-3 sm:ml-4">
                <p className="text-xl sm:text-2xl font-bold text-gray-900">{pastTrips.length}</p>
        ...
        {upcomingTrips.length > 0 && (
          <div className="mb-6 sm:mb-8">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3 sm:mb-4">Upcoming Adventures</h2>
        ...
        {/* All Trips */}
        <div>
          <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-3 sm:mb-4">All Trips</h2>
          {trips.length === 0 ? (
            <div className="bg-white/80 backdrop-blur-sm rounded-xl p-6 sm:p-12 text-center border border-white/20">
              ...
              <p className="text-gray-600 mb-4 sm:mb-6">Start planning your first adventure!</p>
              <button
                onClick={() => setIsCreateModalOpen(true)}
                className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-4 py-2 sm:px-6 sm:py-3 rounded-lg hover:from-blue-700 hover:to-purple-700 transition-all duration-200"
              >
...
const TripCard: React.FC<TripCardProps> = ({ trip }) => {
  ...
  return (
    <Link to={`/planner/${trip.id}`}>
      <div className="bg-white/80 backdrop-blur-sm rounded-xl p-4 sm:p-6 border border-white/20 shadow-sm hover:shadow-lg transition-all duration-200 group">
        ...
        <p className="text-gray-600 text-xs sm:text-sm mb-3 sm:mb-4 line-clamp-2">{trip.description}</p>
